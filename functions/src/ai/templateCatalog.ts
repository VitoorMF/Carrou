...
export function resolveTemplateFromPrompt(prompt: string): TemplateCatalogItem {
    const p = String(prompt || "").toLowerCase();

    if (/premium|luxo|alto padr[aã]o|sofisticado|est[eé]tica|arquitetura/.test(p)) {
        return findTemplateById("luxuryMinimal") ?? getDefaultTemplate();
    }

    if (/advogado|jur[ií]dico|contador|contabilidade|imposto|m[eé]dico|cl[ií]nica|consultoria|empresa|b2b/.test(p)) {
        return findTemplateById("editorial3D") ?? getDefaultTemplate();
    }

    if (/personal|academia|fitness|treino|marketing|instagram|creator|viral|engajamento/.test(p)) {
