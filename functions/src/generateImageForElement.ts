...
function findElementRef(slide: any, targetId: string): ElementRef | null {
    ...
    if (Array.isArray(canvasElements)) {
        ...
        if (eIndex >= 0) return { mode: "canvas", elements: canvasElements, eIndex };
    }

    const layers = slide?.layers ?? {};
    const layerNames: Array<"background" | "atmosphere" | "content" | "ui"> = [
        "background", "atmosphere", "content", "ui",
    ];
    ...
    for (const layerName of layerNames) {
        ...
        if (eIndex >= 0) return { mode: "layer", layerName, elements: layerEls, eIndex };
    }
...
function getProjectTemplateId(projectData: any): string {
    return String(
...
function findStreetwearSharedPair(
    project: any,
    slidesList: any[],
    slideIndex: number,
    element: any,
    ...
    const isSharedStreetwearHero =
        templateId === "streetwearPro"
        && slideIndex >= 0
        && slideIndex <= 3
    ...
    if (!pairSlide) return null;

    const pairRef = findElementRef(
        pairSlide,
        String(element.id).replace(`_${slideIndex}`, `_${pairSlideIndex}`),
    );
...
export const generateImageForElement = onRequest(
    ...
    (req, res) => {
        return corsHandler(req, res, async () => {
            // variables shared between try and catch
            let ref: FirebaseFirestore.DocumentReference | undefined;
            let slides: any[] = [];
            let sIndex = -1;
            let renderCarousel: any = null;
            let sharedPair: ReturnType<typeof findStreetwearSharedPair> = null;
            let elementId = "";
            ...
            try {
                ...
                renderCarousel = project?.renderCarousel ?? null;
                slides = Array.isArray(renderCarousel?.slides)
                    ? renderCarousel.slides
                    : (project.slides ?? []);

                sIndex = slides.findIndex((s: any) => s.id === slideId);
                ...
                sharedPair = findStreetwearSharedPair(project, slides, sIndex, el);

                if (sharedPair?.pairEl?.url || sharedPair?.pairEl?.src) {
                    const sharedSrc = String(sharedPair.pairEl.url ?? sharedPair.pairEl.src ?? "");

                    writeElement(slide, elRef, { ...el, url: sharedSrc, src: sharedSrc, status: "ready" });
                ...
                if (el.url || el.src) {
                    ...
                    if (!exists) return res.json({ ok: true, src: currentSrc, cached: true });
                    ...
                    if (currentSrc !== stableSrc) {
                        writeElement(slide, elRef, { ...el, url: stableSrc, src: stableSrc, status: "ready" });

                        if (sharedPair) {
                            writeElement(slides[sharedPair.pairSlideIndex], sharedPair.pairRef, {
                                ...sharedPair.pairEl, url: stableSrc, src: stableSrc, status: "ready",
                            });
                        }
                ...
                if (sharedPair) {
                    writeElement(slides[sharedPair.pairSlideIndex], sharedPair.pairRef, {
                        ...sharedPair.pairEl, status: "pending",
                    });
                }
                ...
                const encodedPath = encodeURIComponent(path);
                const src = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodedPath}?alt=media&token=${downloadToken}`;
                ...
                const currentEl = refreshedRef ? refreshedRef.elements[refreshedRef.eIndex] : el;

                if (refreshedRef) {
                    writeElement(slides[sIndex], refreshedRef, { ...currentEl, url: src, src, status: "ready" });
                }
                ...
                if (sharedPair) {
                    ...
                    if (refreshedPairRef) {
                        writeElement(slides[sharedPair.pairSlideIndex], refreshedPairRef, {
                            ...currentPairEl, url: src, src, status: "ready",
                        });
                    }
                }
                ...
                try {
                    if (ref && sIndex >= 0 && slides.length > 0 && elementId) {
                        ...
                        if (sharedPair) {
                            const failedPairRef = findElementRef(
                                slides[sharedPair.pairSlideIndex],
                                sharedPair.pairEl.id,
                            );
                            if (failedPairRef) {
